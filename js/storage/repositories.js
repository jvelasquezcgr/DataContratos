...
export class Repository {
  ...
  putMany(entities) {
    return withStore(this.storeName, 'readwrite', store => { entities.forEach(e => store.put(e)); return entities.length; });
  }
}
...
export const saveDocumentBlob = (documentId, blob) =>
  repositories.documentBlobs.put({ id: documentId, blob, updatedAt: new Date().toISOString() });

export const getContractCaseByKey = key =>
  repositories.contractCases.byIndex('byContractKey', key).then(rows => rows[0] || null);

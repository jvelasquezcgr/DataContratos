...
function normalizeSecopDocument(row){
  return {origin:DocumentOrigin.SECOP,externalId:row.id_documento||null,name:row.nombre_archivo||'Documento sin nombre',
    description:row.descripci_n||'',source:row.__sourceLabel||'SECOP',sourceUrl:row.url_descarga_documento?.url||'',
    uploadedAt:row.fecha_carga||null,secopMetadata:{datasetId:row.__datasetId}};
}
